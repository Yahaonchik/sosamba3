...
const Page = () => {
  ...
  const anchors = {
    title: '�� стиральной машины не открывается дверца:',
    items: [
  ...
  const sections = [
    {
      ...
      content: `<p>Многие модели оснащены функцией «Child Lock», которая предотвращает случайное открытие люка во время работы. Если эта блокировка активна, дверца не откроется, и на панели обычно горит соответствующая пиктограмма. В таком случае отключите функцию согласно инструкции — не тяните дверцу силой.</p>`
    },
    {
      id: '3',
      ...
      imageCaption: '',
      content: `<p>Машина не разблокирует люк, пока в баке остаётся вода, это предохраняет от подтопления. Если цикл был прерван или не завершился, в барабане может остаться вода и блокиро��ка не снимется. Восстановите нормальный слив и дождитесь окончания цикла — не дергайте дверцу насильно.</p>`
    },
    {
      id: '4',
      ...
      content: `<p>Если слив заблокирован — засорился фильтр, дренажный шланг или сифон — вода остаётся в баке и люк удерживается запертым. Аналогично, при поломке датчика уровня система может «думать», что вода есть. Требуется очистка сливного тракта и/или диагностика датчика.</p>`
    },
    {
      id: '5',
      ...
      content: `<p>Перебои с питанием или ошиб��и модуля управления могут приводить к «замораживанию» замка л��ка: контроллер не подаёт команду на разблокировку. Иногда помогает полная перезагрузка питания, в иных случаях — диагностика и ремонт.</p>`
    },
    {
      id: '5-1',
      ...
      imageAlt: '',
      imageCaption: '',
      content: `<p>Износ механики замка, сломанная ручка или фиксатор — частая причина невозможности открыть люк. Замок может не фиксироваться или проворачиваться «в холостую». Потребуется замена механизма или ручки у мастера.</p>`
    },
  ]
