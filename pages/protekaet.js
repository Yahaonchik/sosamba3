...
const Page = () => {
  ...
  const navigation = {
    prevPage: { href: '/negreetvodu', title: 'Стираль��ая ма��ина не греет воду' },
  ...
  const sections = [
    ...
    },
    {
      ...
      content: `<p>Если после набора воды заметили под стиралкой воду в районе лотка для порошка, причиной часто бывает его засор. Тогда часть воды «уползает» мимо и вытекает наружу. Иногда лоток просто трескается или расслаивается со временем – тогда вода тоже может уходить прямо через него.</p>
      <p><strong>Совет:</strong> регулярно вынимайте и промывайте лоток для порошка и сам патрубок подачи воды в бак. Если давление в водопроводе слишком сильное, воду может «выдавить» наружу – попробуйте немного прикрыть кран подачи. Не используйте слишком много моющего средства и только предназначенные для машин порошки – избыток пены тоже выгоняет влагу наружу. При появлении лужи на полу во время набора воды стоит отключи��ь машинку, собрать бельё и прочистить дозатор: обычно это быстро решает проблему.</p>`
    },
    {
      id: '2-2',
      ...
      imageAlt: '',
      imageCaption: '',
      content: `<p>Наиболее серьёзная причина – это нарушение целостности самого бака (ёмкости) машины. Внутри может образоваться дырка из-за мелких острых предметов – чаще всего стальных косточек от бюстгальтера, проскочивших в барабан. Такие пробоины вызывают неконтролируемый «фонтан»: вода течёт непрерывно и из-под машины в любых режимах. Иногда бак трескается по шву или по износу деформируется, что тоже даёт течь.</p>
      <p><strong>Совет:</strong> к сожалению, отремонтировать корпус бака почти невозможно – его обычно заменяют целиком или вместе с барабаном. Чтоб�� не допустить такого сбоя, тщательно проверяйте и чистите внутренние карманы у белья. Раз в месяц запускайте режим самоочистки с пустым барабаном (если есть) или используйте специальные средства против накипи и загрязнений. Это уменьшит накопление частиц внутри бака и продлит его ресурс.</p>`
    },
    {
      id: '3',
      ...
      content: `<p>В некоторых случаях вода под машиной появляется только во время отжима – это характерно для износа подшипников и сальника барабана. Сальник (уплотнитель) узла вращения постепенно истирается, из-за чего вода попадает внутрь подшипников и выходит наружу. Сначала заметно лишь учащённое включение помпы аквастопа (если он есть), а потом – ржавая или с маслянистыми пятнами вода снизу. При этом машина громко гремит и «гуляет» на отжиме.</p>
      <p><strong>Совет:</strong> осмотреть это своими силами можно только сняв заднюю панель, но замена подшипников – задача мастера: приходится разбирать весь бак. Если вы слышите новый скрежет и вибрацию в сочетании с лужей под стиралкой, лучше как можно скорее вызвать специалиста. Регулярное использование специальных очистителей и держание машины в ровном положении может чуть замедлить износ, но окончательно устранить течь сможет только замена сальника и подшипников.</p>`
    },
    {
      id: '4',
    ...
    },
    {
      ...
      imageAlt: '',
      imageCaption: '',
      content: `<p><strong>1. Проверяйте карманы и бережно укладывайте бельё.</strong> Мелкие детали одежды – пуговицы, заклёпки, монеты – кладите в стир��льные мешки.</p>
      ...
      <p><strong>5. Обеспечьте прочную установку машины.</strong> Стиральная машина должна стоять строго по уровню: если она «гуляет», вибрации ускоряют износ деталей. Регулируйте ножки так, чтобы ма��ина была неподвижна при работе.</p>
